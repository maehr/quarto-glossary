...
- Proper ARIA attributes ensure definitions are announced correctly
- Terms are marked with `role="button"` and `aria-expanded` states
- Definitions use `role="tooltip"` and are linked via `aria-describedby`
...
### Removed Features
The "hover" popup mode has been removed as it relied on native browser tooltips that do not meet accessibility standards. All functionality is now available through the accessible "click" mode.
